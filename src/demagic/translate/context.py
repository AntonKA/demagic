"""Translation rules + context-pack builder.

This module has NO LLM-SDK dependency, so the agent-driven flow (`demagic pack`,
`demagic init`) works on a light install. The pydantic-ai Agent that calls a
provider API lives in `agent.py` and is only needed for the optional `[api]`
headless translate mode.
"""
